...
export async function placeMarketOptionOrder(
  optionSymbol: string,
  contracts: number,
  ...
  try {
    ...
    const qty = Math.max(1, Math.min(2000, Number(contracts || 1)));
    if (!sym) return { ok: false, status: 400, error: 'optionSymbol required' };
