...
import { analyzeTechnicals, scoreTechnicals, realizedVol } from './technicals.js';
...
export function scoreVolatility(chainData, closePrices, direction) {
  ...
  // For IV Rank, we'd need historical IV data. Approximate using chain data
  // In production, cache daily IV snapshots to calculate true 52-week rank
  // For now, use a heuristic based on IV vs realized vol
  const ivRank = currentIV ? Math.min(100, Math.max(0, (currentIV * 100 - 15) * 1.5)) : 50;

  const volPremium = currentIV && rv20 ? (currentIV * 100 - rv20) : 0;
...
// ============================================================
export function scoreOpportunity({ flowData, chainData, closePrices, earningsData, newsData, ticker, stockPrice }) {
  // Layer 1: Flow Intent
  const flow = scoreFlowIntent(flowData);
  if (flow.score === 0) return null; // No speculative flow = no card
  ...
  if (!suggestedPlay) return null; // couldn't find a valid trade
