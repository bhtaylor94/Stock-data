...
// States:
// ABOVE_EMA     — price >3% above 200 EMA (normal, no flag)
// APPROACHING   — price within 3% above 200 EMA (yellow flag)
// AT_SUPPORT    — price within 1% of 200 EMA (orange flag — wait)
// BELOW_EMA     — price >1% below 200 EMA (red flag — avoid)
// CONFIRMED     — bounced and held above EMA for 2 days (green flag)
// FAILED        — broke below EMA after testing (red flag)
...
export function calculateEma200Proximity(closePrices, volumes, livePrice) {
  ...
  // Use live price if available (from chain data), otherwise last close
  const currentPrice = livePrice || closePrices[closePrices.length - 1];
  const ema200Rounded = Math.round(ema200 * 100) / 100;
  const distance = ((currentPrice - ema200) / ema200) * 100;
  const distanceRounded = Math.round(distance * 10) / 10;

  // Determine state
  let state, message, color, action;

  if (currentPrice > ema200 * 1.03) {
    state = 'ABOVE_EMA';
    message = `Price is ${distanceRounded}% above 200 EMA ($${ema200Rounded}). No support concerns.`;
    color = null; // no banner color needed
    action = null;
  } else if (currentPrice > ema200 * 1.01) {
    state = 'APPROACHING';
    message = `Price is approaching 200 EMA support at $${ema200Rounded}. Current price: $${currentPrice} (${distanceRounded}% above). Consider waiting for a test and bounce before entering.`;
    color = '#eab308'; // yellow
    action = 'CAUTION';
  } else if (currentPrice >= ema200 * 0.99) {
    state = 'AT_SUPPORT';
    message = `Price is testing 200 EMA support at $${ema200Rounded}. DO NOT BUY YET. Wait 1-2 days for the price to close above $${ema200Rounded} to confirm the bounce.`;
    color = '#f97316'; // orange
    action = 'WAIT';
  } else {
    state = 'BELOW_EMA';
    message = `Price has broken below 200 EMA support at $${ema200Rounded}. AVOID this entry. Wait for price to reclaim $${ema200Rounded} and hold above it for 2 days before reconsidering.`;
    color = '#ef4444'; // red
    action = 'AVOID';
  }

  // Check for bounce confirmation (last 3 days of closes)
  let bounceStatus = null;
  if (state === 'AT_SUPPORT' || state === 'APPROACHING') {
    const recentCloses = closePrices.slice(-3);
    const daysAboveEma = recentCloses.filter(c => c > ema200).length;

    if (daysAboveEma >= 2 && recentCloses[recentCloses.length - 1] > ema200 && recentCloses[recentCloses.length - 2] > ema200) {
      // Check volume confirmation if available
      let highVolumeConfirm = false;
      if (volumes && volumes.length >= 21) {
        const avgVol = volumes.slice(-21, -1).reduce((a, b) => a + b, 0) / 20;
        const recentVol = volumes[volumes.length - 1];
        highVolumeConfirm = recentVol > avgVol * 1.2;
      }

      state = 'CONFIRMED';
      const lastTwo = recentCloses.slice(-2).map(c => `$${c.toFixed(2)}`).join(', ');
      message = `Bounce confirmed. Price held above 200 EMA ($${ema200Rounded}) for 2 consecutive days. Closes: ${lastTwo}.${highVolumeConfirm ? ' Volume confirms the bounce.' : ''}`;
      color = '#22c55e'; // green
      action = 'ENTRY_CONFIRMED';
      bounceStatus = { confirmed: true, highVolume: highVolumeConfirm, closes: recentCloses.slice(-2) };
    } else if (recentCloses[recentCloses.length - 1] < ema200 * 0.99) {
      state = 'FAILED';
      message = `Bounce failed. Price broke below 200 EMA ($${ema200Rounded}). Closed at $${currentPrice.toFixed(2)}. Avoid this entry.`;
      color = '#ef4444';
      action = 'AVOID';
      bounceStatus = { confirmed: false, failed: true };
    } else if (daysAboveEma === 1) {
      bounceStatus = { confirmed: false, pending: true, daysConfirmed: 1 };
      message += ` Day 1 confirmed. Waiting for Day 2 close above $${ema200Rounded}.`;
    }
  }

  // Grade modifier
  let gradeModifier = 0;
  if (state === 'CONFIRMED') gradeModifier = 1;      // upgrade by 1 letter
  else if (state === 'APPROACHING') gradeModifier = 0; // no change
  else if (state === 'AT_SUPPORT') gradeModifier = -0.5; // slight downgrade
  else if (state === 'BELOW_EMA' || state === 'FAILED') gradeModifier = -1; // full downgrade

  return {
    ema200: ema200Rounded,
    currentPrice,
...
export function adjustGrade(grade, modifier) {
  ...
  const newIdx = Math.max(0, Math.min(grades.length - 1, idx - modifier)); // - because lower index = better
