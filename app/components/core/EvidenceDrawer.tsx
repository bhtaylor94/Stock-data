...
function renderAllSections(data: any) {
  ...
  // 7. SIGNAL AGREEMENT / DISAGREEMENT ANALYSIS
  try {
    if (data?.meta?.tradeDecision) {
      const decision = data.meta.tradeDecision;
      const isNoTrade = decision.action === 'NO_TRADE';
      
      // Check for contradictions between main rating and individual signals
      const analystsBullish = data?.analysts?.consensus === 'STRONG BUY' || data?.analysts?.consensus === 'BUY';
      const fundamentalsStrong = data?.analysis?.fundamental?.score >= 6;
      const technicalsStrong = data?.analysis?.technical?.score >= 6;
      
      const hasContradiction = isNoTrade && (analystsBullish || fundamentalsStrong || technicalsStrong);
      
      if (hasContradiction) {
        sections.push(
          <section key="signal-disagreement">
            <h3 className="text-lg font-semibold text-white mb-3">⚠️ Signal Disagreement</h3>
            <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/30 space-y-3">
              <p className="text-sm text-amber-200 font-medium">
                Recommendation is NO TRADE because signals do NOT agree:
              </p>
                ...
                {analystsBullish && (
                  <div className="flex items-start gap-2">
                    ...
                    <span className="text-slate-300">
                      Analysts are bullish: {data.analysts.consensus} ({data.analysts.buyPercent}% buy rating)
                    </span>
                  </div>
                )}
                {fundamentalsStrong && (
                  <div className="flex items-start gap-2">
                    <span className="text-emerald-400">✓</span>
                    <span className="text-slate-300">
                      Fundamentals are strong: {data.analysis.fundamental.score}/{data.analysis.fundamental.maxScore}
                    </span>
                  </div>
                )}
                {!fundamentalsStrong && (
                  <div className="flex items-start gap-2">
                    ...
                    <span className="text-slate-300">
                      Fundamentals are weak: {data.analysis.fundamental.score}/{data.analysis.fundamental.maxScore}
                    </span>
                  </div>
                )}
                {technicalsStrong && (
                  <div className="flex items-start gap-2">
                    <span className="text-emerald-400">✓</span>
                    <span className="text-slate-300">
                      Technicals are strong: {data.analysis.technical.score}/{data.analysis.technical.maxScore}
                    </span>
                  </div>
                )}
                {!technicalsStrong && (
                  <div className="flex items-start gap-2">
                    <span className="text-red-400">✗</span>
                    <span className="text-slate-300">
                      Technicals are weak: {data.analysis.technical.score}/{data.analysis.technical.maxScore}
                    </span>
                  </div>
                )}
              </div>
              <p className="text-xs text-slate-400 pt-2 border-t border-amber-500/20">
                💡 Our system requires at least 4 of 6 signals to agree for a trade recommendation. 
                When signals conflict, we recommend NO TRADE to avoid high-risk situations.
              </p>
              {decision.rationale && decision.rationale.length > 0 && (
                <div className="pt-2 border-t border-amber-500/20">
                  <p className="text-xs text-slate-500 mb-1">Specific reasons:</p>
                  {decision.rationale.map((r: string, i: number) => (
  ...
  } catch (e) { console.error('Signal disagreement error:', e); }
